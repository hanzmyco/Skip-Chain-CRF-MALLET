...
//				dumpValues ("Expectations", expectations);
	    ...
	    if (buf.length != numParameters)
				throw new IllegalArgumentException
					("Incorrect length buffer to getValueGradient(). Expected " + numParameters + ", received " + buf.length);
...
//				dumpAssnForClique (assn, clique);
	...
	public static class BigramTemplate extends ACRF.SequenceTemplate {
																				...
																				LabelsSequence lblseq)
		{
		  ...
		  //System.out.println(lblseq.toString());
		  //System.out.println(fvs.toString());
        ...
        //String name=fv.toString(true);
        
        //String feature[]=name.split("\\r?\\n");
        //String f[]=feature[0].split(" ");
        

        
        /*
        for (int inde=0;inde<f.length;inde++){
          int lenn=f[inde].length();
          if (f[inde].substring(0,2).equals("W=") && f[inde].charAt(lenn-2)!='@'&& f[inde].charAt(lenn-3)!='@'){
            System.out.println(i);
            System.out.println(f[inde]);
            break;
          }
          //System.out.println(i);
          //System.out.println(feature[inde]);
        }
        */
	...
	public static class PairwiseFactorTemplate extends ACRF.SequenceTemplate {
		...
		public PairwiseFactorTemplate (int factor0)
		{
        ...
        for (int j=i+1;j<lblseq.size();j++){

          FeatureVector fvj = fvs.getFeatureVector (j);
        String name1=fvj.toString(true);
        
        //String featurej[]=name1.split("\\r?\\n");
        String featurej[]=name1.split("\\n");
        
        String fj[]=featurej[0].split(" ");
        String namej="";

        for (int inde=0;inde<fj.length;inde++){
          int lenn=fj[inde].length();
          if (fj[inde].substring(0,2).equals("W=") && fj[inde].charAt(lenn-2)!='@'&& fj[inde].charAt(lenn-3)!='@'){
        ...
        Variable v2 = graph.getVarForLabel (j, factor0);
        ...
        assert v2 != null : "Couldn't get label factor "+factor0+" time "+j;        
  ...
  public static ACRF makeFactorial (Pipe p, int numLevels)
  {
    ...
    for (int i = 0; i < numLevels; i++) {
      ...
      if (i+1 < numLevels)
        t.add (new PairwiseFactorTemplate (i));
    }
